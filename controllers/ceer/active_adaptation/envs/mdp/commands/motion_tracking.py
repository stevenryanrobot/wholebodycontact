...
class MotionTrackingCommand(Command):
    def __init__(self, env, dataset: dict,
        ...
        # ProACT active-probe: commanded body-frame planar velocity in static
        # mode (default zero = stand still). A collector can oscillate this to
        # make the robot weight-shift/sway (unloading a leg to break stance
        # masking) without touching the policy.
        self.static_linvel_b = torch.zeros(self.num_envs, 2, device=self.device)
...
class MotionTrackingCommand_impedance(MotionTrackingCommand):
    def __init__(
        ...
        **kwargs,
        ...
        with torch.device(self.device):
            ...
            self.force_origin_w = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32) # 弹簧锚点(原点)在世界坐标系下的位置 [N, M, 3]
            self.force_dir_w = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32)       # 弹簧拉力方向(单位向量),世界系:原点->参考关键点
            # spring force
            # 实际施加到末端 body 上的弹簧力。world 系给物理引擎/reward 用,body 系喂给 observation(本体受力感知)
            self.force_applied_w = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32) # 施加力,世界坐标系 [N, M, 3]
            self.force_applied_b = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32) # 同一个力,旋转到机器人 root/body 坐标系
            self.force_expected_w = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32) # 期望(目标)施加力,世界系 —— reward 里和实际力比较
            self.force_expected_b = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32) # 期望施加力,body 系
            self.root_compliance_offset_xy = torch.zeros(self.num_envs, 2, dtype=torch.float32)

            # force apply
            self.force_pos_delta = torch.zeros(self.num_envs, self.num_force_bodies, 3, dtype=torch.float32)  # 力的作用点相对 body 原点的偏移(body 系,≤5cm),用来产生力臂/扭矩
            self.force_apply_buffer = torch.zeros(self.num_envs, self.asset.num_bodies, 3, dtype=torch.float32)   # 施加给仿真的力,索引覆盖所有 body [N, num_bodies, 3]
            self.torque_apply_buffer = torch.zeros(self.num_envs, self.asset.num_bodies, 3, dtype=torch.float32)  # 施加给仿真的力矩(由偏移点的力臂产生)
    ...
    def force_update_origin_and_target(self):
        ...
        # 弹簧锚点存在 root 系(force_origin_tl 随时间在起点->终点之间线性插值),这里转到世界系
        ...
        # 参考关键点(运动数据里末端应到的位置)转到 root 系,弹簧方向即朝向它
        ...
        # 弹簧方向 = 从锚点指向参考点(单位向量),先在 root 系算再转世界系
    ...
    def force_apply(self, substep):
        # admittance(柔顺/弹簧)主模式:把一根虚拟弹簧的力施加到末端 body 上
        # 每个 substep 先清零所有力/力矩缓存
        self.force_applied_w.zero_()
        self.force_applied_b.zero_()
        self.force_apply_buffer.zero_()
        self.torque_apply_buffer.zero_()

        pos_w = self.asset.data.body_pos_w[:, self.force_apply_idx_asset, :]   # 末端 body 当前世界坐标位置 [N, M, 3]
        quat_w = self.asset.data.body_quat_w[:, self.force_apply_idx_asset, :] # 末端 body 当前世界姿态(四元数)

        # 弹簧力 = 刚度 × (锚点->当前位置的位移,投影到弹簧方向),再按 max_force*alpha 限幅
        #   force_origin_w - pos_w : 弹簧锚点到末端的位移向量
        #   project_pos_diff(...)  : 只取沿 force_dir_w 方向的分量
        #   force_kp_scaled        : 弹簧刚度(含 enable 掩码与 alpha)
        #   clamp_norm(..., 30N)   : 力大小封顶 max_force(默认30N)*force_alpha
        ...
        # 把世界系的力旋转到机器人 root 坐标系,供 observation 使用
        ...
        # 力的真实作用点相对 body 原点偏移 force_pos_delta(转到世界系),由此产生力矩 = delta × F
        # 等效于把力施加在 body_pos + delta_w 这个点上
        ...
        # override original force apply —— 关闭默认外力通道,改用本类的世界系施力路径
        self.asset.has_external_wrench = False
        self.force_apply_world = True

    # this function is for baseline (non-compliant perturbation)
    # baseline 模式:不走弹簧,直接往最后两个 body 施加随机扰动力 perturb_force(非柔顺对照组)
    ...
    def force_apply_net_pull(self, substep: int):
        # net_pull 模式:模拟绳/网拉力,把 net_pull_force_w 施加到指定 body 上
        self.force_applied_w.zero_()
        self.force_applied_b.zero_()
        self.force_apply_buffer.zero_()
        self.torque_apply_buffer.zero_()
    ...
    def _static_root_command(self):
        ...
        target_linvel_b_xy = self.static_linvel_b
