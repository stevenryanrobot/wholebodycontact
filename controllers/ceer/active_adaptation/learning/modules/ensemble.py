...
class EnsembleModule(TensorDictModuleBase):
    ...
    def __init__(
        ...
        self.vmapped_forward = torch.vmap(self._func_module_call, (-1, 0))
    ...
    def _maybe_expand(self, tensor: torch.Tensor, shape):
        if tensor.shape[:len(shape)] != shape:
            tensor = tensor.unsqueeze(len(shape)-1)
            tensor = tensor.expand(shape + tensor.shape[len(shape):])
        return tensor
        
    def forward(self, tensordict: TensorDict) -> TensorDict:
        target_shape = (*tensordict.shape, self.num_copies)
        input = (
            tensordict.select(*self.in_keys)
            .apply(lambda tensor: self._maybe_expand(tensor, target_shape), batch_size=target_shape)
        )
        results = self.vmapped_forward(input, self.params_td)
        results = results.permute(*range(1, results.ndim), 0).contiguous()
        tensordict.update(results, keys_to_update=self.out_keys)
        return tensordict
